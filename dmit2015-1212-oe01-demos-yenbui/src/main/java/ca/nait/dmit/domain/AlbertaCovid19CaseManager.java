...
public class AlbertaCovid19CaseManager {
    ...
    public AlbertaCovid19CaseManager() throws IOException {
        try(var reader = new BufferedReader(new InputStreamReader(getClass().getResourceAsStream(
            ...
            while ((lineText = reader.readLine()) != null) {
                ...
                String[] values = lineText.split(DELIMITER, -1); // the -1 limit allows for any number of fields

                // colums order of fields
                // 0 - "Id"
                // 1- "Date Reported"
                ...
                currentCase.setAshZone(values[2].replaceAll("\"", ""));
    ...
    public List<String> findDistincAshZone() {
        return albertaCovid19CaseList
                .stream()
                .map(AlbertaCovid19Case::getAshZone)
                .distinct()
                .sorted()
